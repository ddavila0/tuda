...
@fetch_wrapper(tag="xrootd_data", cache="socal")
def fetch_data_xrootd(hdfs_path):
    ...
    df = (jobreports
            ...
            # 'Namespace' filter
            .filter(col('data.file_lfn').rlike('/store/data*'))
...
@fetch_wrapper(tag="xrootd_user", cache="socal")
...
@fetch_wrapper(tag="xrootd_mc", cache="socal")
def fetch_mc_xrootd(hdfs_path):
    """Fetch HDFS XRootD records from a particular path"""
    # DBS file info
    csvreader = spark.read.format("com.databricks.spark.csv").option("nullValue","null").option("mode", "FAILFAST")
    dbs_base="/project/awg/cms/CMS_DBS3_PROD_GLOBAL/current"
    ...
    df = (jobreports
            ...
            .filter(col('data.vo') == "cms")
            # 'Namespace' filter
            .filter(col('data.file_lfn').rlike('/store/mc*'))
            # Get the job id from DBS  
            .join(dbs_files, col('data.file_lfn')==col('f_logical_file_name'))
            # Select columns to save
            .select(
